...
void Voxel::ComputeParticles(double radius_1, double solids_loading) {
    ...
    // total number of host _nodes for particles
    ...
    while ((tot_part_nodes < n_particle_nodes) && (counter1 < 10000000)){
        ...
        // ensure vectors contain no duplicated _nodes
        UniqueVec(_particles_ind);
        UniqueVec(_particle_interfacial_nodes);
        ...
        // update total number of particle _nodes
...
double Voxel::IRate(std::vector<double> &conc_PI, double _I0, double z, int node) const {
    if (_material_type[node] == 1 && _timer < _uvt){
        // material is ugap
        // return (-_phi * _eps * _I0 * conc_PI[node] * exp( -_eps*conc_PI[node]*z) / 2);
        return (-_phi * _eps * _I0 * conc_PI[node] * exp( -_eps*conc_PI[node]*z) / 2);
    }
    ...
    */
    if (_material_type[node] == 1){
       ...
       // compute chemical diffusion as a function of variable diffusivity
        ...
        return (_phi*_eps*_I0*conc_PI[node]*exp(-_eps*conc_PI[node]*z) - _k_i[node]*conc_PIdot[node]*conc_M[node] + diffuse);
    }
    
    else if (_material_type[node] == 2){
        // material is interface
        return (_phi*_eps*_I0*conc_PI[node]*exp(-_eps*conc_PI[node]*z) - _k_i[node]*conc_PIdot[node]*conc_M[node]);
    }
    ...
    while (error > _tol){
        ...
        for (int node = 0; node < _N_VOL_NODES; node++){
            ...
            if (   i != 0 && i != (_nodes-1)
                ...
                // solve equation 1
                c_PI_1[node] =   _c_PI[node] + _dt * (  (1-psi) * IRate(c_PI_0, _I0, depth, node)
                                                        +   psi   * IRate(_c_PI, _I0, depth, node));

                err_step += SquaredDiff(c_PI_0[node], c_PI_1[node]);


                // solve equation 2
                c_PIdot_1[node] = _c_PIdot[node] + _dt * (   (1-psi) * PIdotRate(c_PIdot_0, c_PI_0, c_M_0, _I0, depth, node)
                                                            +   psi   * PIdotRate(_c_PIdot, _c_PI, _c_M, _I0, depth, node));
                ...
                // solve equation 5
                theta_1[node] = _theta[node] + _dt* (   (1-psi) * TempRate(theta_0, c_M_0, c_Mdot_0, c_PI_0, _c_PIdot, _I0, node)
                                                    +   psi   * TempRate(_theta, _c_M, _c_Mdot, _c_PI, _c_PIdot, _I0, node));
            ...
            // BOUNDARY NODES
                ...
                c_PI_1[node] = _c_PI[node]
                                + _dt*(  (1-psi) * IRate(c_PI_0, _I0, depth, node)
                                        +   psi   * IRate(_c_PI, _I0, depth, node));
                err_step +=   SquaredDiff(c_PI_0[node], c_PI_1[node]);
...
void Voxel::Config2File(double _dt){
    
    // write to file
    _print_sim_config.open(file_path + "sim_config/sim_config.txt");
    ...
    _print_sim_config << "_h: " << _h << std::endl;
    _print_sim_config << "_dt: " << _dt << std::endl;
    _print_sim_config << "Diffusion CFL: " << _Dm0 * _dt / _h / _h << std::endl;
    _print_sim_config << "Thermal CFL: " << _dt / _h / _rho_UGAP / _Cp_nacl << std::endl;
...
void Voxel::Density2File(){
    // write to file
    _print_density.open(file_path + "_density/_density.vtk");
...
void Voxel::AvgConcentrations2File(int counter, 
    ...
    for (int node = 0; node < _N_VOL_NODES; node++){
        ...
        // compute average temperature related values over all _nodes
        avg_diff_theta  += _diff_theta[node];
        avg_tot_theta   += theta_next[node];

        // compute average diffusivity of temperature top and bottom _nodes
        ...
        // compute average reaction related values over resin _nodes
        if (_material_type[node] == 1){
    ...
    // open file
    if (counter == 0){std::string file_avg_concentrations = file_path + "python_plotting/avg_concentration_simID_" + std::to_string(_sim_id) + ".csv";
...
void Voxel::NonBoundaries2File( int counter, 
    ...
    // writes only non-boundary _nodes to file
    ...
    // loop through all _nodes
    for (int node = 0; node < _N_VOL_NODES; node++){
        
        // only write internal _nodes
        Node2Coord(node, _current_coords); 
...
void Voxel::Simulate(int method, int save_voxel){
    ...
    // time discretization -> [0., _dt, 2*_dt, ..., T]
